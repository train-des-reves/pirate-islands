...
La coordination du dépôt se fait par une tâche planifiée toutes les cinq minutes, jamais par un agent surveillant permanent. Le `surveillant` fusionne les responsabilités de contrôleur et d'advisor opérationnel : il possède la file d'issues, les dépendances, les dispatchs et les fusions, mais aussi l'inspection de santé des exécutants, le diagnostic des blocages connus et les nudges ciblés. Son profil de référence est `gpt-5.6-luna` avec effort `high`.

À chaque passage, le surveillant réconcilie d'abord l'état réel : issues, PR, branches, tâches, worktrees et CI. Un checkpoint, un résumé de conversation, `findings.md` ou un registre local peut être périmé et ne doit jamais déclencher seul un dispatch, une relance, un archivage ou une fusion. Une issue déjà fusionnée ou encore possédée par une branche, une PR ou une tâche active n'est jamais recréée.

Le surveillant maintient entre les cycles un registre opérationnel local dans `.codex/etat-surveillant/`, ignoré par Git. Ce registre contient au minimum issue, tâche, branche, worktree, modèle, dépendances, dernier statut, dernier SHA/PR observé, dernière intervention et prochain point à vérifier. Il accélère la reprise mais ne remplace jamais la réconciliation fraîche. `findings.md` est le registre durable des incidents distincts, signatures de panne, causes et récupérations réutilisables ; les heartbeats ordinaires et snapshots courants de la file n'y sont pas autoritaires.

Après la réconciliation, la tâche planifiée lance un exécuteur isolé pour chaque issue prête, dans la limite de concurrence autorisée. Tout exécuteur utilise le modèle exact `deepseek/deepseek-v4-flash-vision-exp` avec l’effort de raisonnement `high`. Aucun modèle `github-copilot/*` n’est autorisé, y compris pour une relance ou une revue. Lorsqu'il termine, se bloque ou change d’étape, il consigne immédiatement son statut dans sa propre tâche ; il ne communique pas directement avec le surveillant. Le surveillant lit ces tâches au passage planifié suivant. Un statut ordinaire tient sur une seule ligne au format `thème — état`, sans logs, justification ni détail technique.

À chaque cycle, le surveillant inspecte chaque exécutant actif : dernier statut, erreurs récentes, avancement, head/PR connu et symptômes comparés aux incidents de `findings.md`. Il distingue attente normale, travail actif, blocage connu, boucle improductive, tâche disparue et état ambigu. Il n'envoie une consigne que si elle apporte une information ou une décision nouvelle. Un nudge doit être court, ciblé, préserver le travail déjà produit et indiquer la récupération connue ou la prochaine vérification bornée. Ne jamais renvoyer la même consigne à chaque heartbeat ni redémarrer une tâche pour un simple silence.

Quand une signature de panne connue réapparaît, utiliser d'abord la récupération déjà documentée. Quand l'incident est nouveau, le surveillant diagnostique une fois, consigne les faits et l'intervention dans son registre local, puis ajoute à `findings.md` seulement les éléments génériques réutilisables. Une ambiguïté produit ou une décision d'architecture nouvelle est escaladée à l'humain ; le surveillant ne l'invente pas pour débloquer artificiellement un lead.

Chaque lead délègue, une fois sa PR distante publiée, la revue indépendante à un sous-agent avec le modèle exact `deepseek/deepseek-v4-pro` et l'effort `max`. Le lead orchestre directement la boucle avec ce sous-agent : le même reviewer est réutilisé pour chaque nouveau head de cette PR, sans reviewer parallèle ni duplication. Le lead ne réalise pas lui-même la revue. Un verdict produit par `github-copilot/*` est invalide pour le `Go final` et doit être remplacé par une revue deepseek-v4-pro sous-agent. Le sous-agent envoie son verdict ou son diagnostic au lead ; celui-ci le consigne dans sa tâche pour lecture au prochain cycle. Tout statut significatif — blocage, nouveau head ou résultat de barrière — doit rester une seule ligne `thème — état`, sans détail parasite ; aucune communication directe au surveillant n'est attendue. Les messages formels `Entrée en revue` et `Go final` conservent uniquement leur checklist obligatoire (issue, PR, head, CI, preuve, périmètre, verdict et mergeabilité) et sont consignés dans la tâche. Le lead doit annoncer explicitement l'`Entrée en revue` en précisant issue, PR, head, CI, preuve et périmètre contrôlé, puis traiter chaque retour actionnable et relancer les barrières touchées. Quand la CI GitHub est verte, que le reviewer deepseek-v4-pro sous-agent est favorable sur le head exact, que la preuve et les assets correspondent et que la PR est fusionnable, le lead consigne un `Go final` explicite dans sa tâche avec ces éléments. Pour une PR déjà en revue lors de l'entrée en vigueur de cette règle, le lead reprend le sous-agent deepseek-v4-pro existant au lieu d'en créer un second, à condition qu'il n'utilise pas `github-copilot/*`.

Le surveillant n'attend jamais activement un exécuteur ou un reviewer et ne fait pas de polling ad hoc de leurs tâches. Le passage planifié est le heartbeat. Il traite les actions locales sûres, publie seulement les consignes utiles, puis rend la main ; les statuts consignés, les messages formels `Entrée en revue`/`Go final`, les findings et les blocages sont lus au passage planifié suivant.
...
Le surveillant ne fusionne une PR qu'après lecture du `Go final` explicite consigné par son exécuteur, en plus d'une CI entièrement verte, d'une revue `deepseek/deepseek-v4-pro` `max` favorable sur le head exact, d'une preuve cohérente et d'un état fusionnable. Juste avant fusion il réconcilie à nouveau head, issue, CI, mergeabilité et preuve ; un `Go final` ancien ne remplace pas cette vérification. Il met ensuite `main` à jour et vérifie que le commit de tête et l'état de fusion sont cohérents. Ne jamais fusionner une PR sans ces conditions, ni auto-approuver une PR. Après confirmation de la fusion, archiver la tâche de l'exécuteur et celle du reviewer, mettre à jour le registre local, puis fermer l'issue correspondante ; ne fermer aucune issue avant que la PR soit effectivement fusionnée.
