...
Chaque lead délègue, une fois sa PR distante publiée, la revue indépendante à un sous-agent avec le modèle exact `gpt-5.6-luna` et l'effort `max`. Le lead orchestre directement la boucle avec ce sous-agent : le même reviewer est réutilisé pour chaque nouveau head de cette PR, sans reviewer parallèle ni duplication. Le lead ne réalise pas lui-même la revue. Un verdict produit par `github-copilot/*` est invalide pour le `Go final` et doit être remplacé par une revue Luna sous-agent. Le sous-agent envoie son verdict ou son diagnostic au lead ; celui-ci le consigne dans sa tâche pour lecture au prochain cycle. Tout statut significatif — blocage, nouveau head ou résultat de barrière — doit rester une seule ligne `thème — état`, sans détail parasite ; aucune communication directe au surveillant n'est attendue. Les messages formels `Entrée en revue` et `Go final` conservent uniquement leur checklist obligatoire (issue, PR, head, CI, preuve, périmètre, verdict et mergeabilité) et sont consignés dans la tâche. Le lead doit annoncer explicitement l'`Entrée en revue` en précisant issue, PR, head, CI, preuve et périmètre contrôlé, puis traiter chaque retour actionnable et relancer les barrières touchées. Quand la CI GitHub est verte, que le reviewer Luna sous-agent est favorable sur le head exact, que la preuve et les assets correspondent et que la PR est fusionnable, le lead consigne un `Go final` explicite dans sa tâche avec ces éléments. Pour une PR déjà en revue lors de l'entrée en vigueur de cette règle, le lead reprend le sous-agent Luna existant au lieu d'en créer un second, à condition qu'il n'utilise pas `github-copilot/*`.
