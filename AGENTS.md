...
La coordination du dépôt se fait par une tâche planifiée toutes les cinq minutes, jamais par un agent surveillant permanent. À chaque passage, la tâche planifiée vérifie les issues, leurs dépendances, les PR et les branches, puis lance un exécuteur isolé pour chaque issue prête. Tout exécuteur utilise le modèle direct `gpt-5.6-luna` avec l’effort de raisonnement `max`. Aucun modèle `github-copilot/*` n’est autorisé, y compris pour une relance ou une revue. Lorsqu'il termine, se bloque ou change d’étape, il envoie immédiatement son statut directement au surveillant actif via `send_message_to_thread` (`mcp__codex_app.send_message_to_thread`) ; ne pas passer par un parent, un relais implicite, un canal direct app-server ni supposer qu'un message ordinaire de sous-agent sera routé. Un statut ordinaire tient sur une seule ligne au format `thème — état`, sans logs, justification ni détail technique.

Chaque exécuteur est responsable de lancer lui-même, une fois sa PR distante publiée, la revue indépendante de sa propre PR avec le modèle direct `gpt-5.6-terra` et l'effort `medium`. Il boucle directement avec ce reviewer : le même reviewer est réutilisé pour chaque nouveau head de cette PR, sans reviewer parallèle ni duplication. Un verdict produit par `github-copilot/*` est invalide pour le `Go final` et doit être remplacé par une revue directe Terra. Le reviewer envoie son verdict ou son diagnostic à l'exécuteur ; celui-ci transmet immédiatement le bilan directement au surveillant via `send_message_to_thread`. Tout statut significatif — blocage, nouveau head ou résultat de barrière — doit rester une seule ligne `thème — état`, sans détail parasite ; aucun relais par un parent n'est attendu. Les messages formels `Entrée en revue` et `Go final` conservent uniquement leur checklist obligatoire (issue, PR, head, CI, preuve, périmètre, verdict et mergeabilité). L'exécuteur doit annoncer explicitement l'`Entrée en revue` en précisant issue, PR, head, CI, preuve et périmètre contrôlé, puis traiter chaque retour actionnable et relancer les barrières touchées. Quand la CI GitHub est verte, que le reviewer Terra direct est favorable sur le head exact, que la preuve et les assets correspondent et que la PR est fusionnable, l'exécuteur envoie un `Go final` explicite au surveillant avec ces éléments. Pour une PR déjà en revue lors de l'entrée en vigueur de cette règle, l'exécuteur reprend la tâche Terra existante au lieu d'en créer une seconde, à condition qu'elle n'utilise pas `github-copilot/*`.

Le surveillant n'attend jamais activement un exécuteur ou un reviewer et ne fait pas de polling de leurs tâches. Il traite les actions locales sûres, publie ses consignes, puis rend la main ; les messages explicites `Entrée en revue`, findings, blocages et `Go final` sont les retours attendus, et la tâche planifiée reprend la coordination au passage suivant si aucun message n'est encore arrivé.
