...
Chaque exécuteur est responsable de lancer lui-même, une fois sa PR distante publiée, la revue indépendante de sa propre PR avec `gpt-5.6-terra` et l'effort `medium`. Il boucle directement avec ce reviewer : le même reviewer est réutilisé pour chaque nouveau head de cette PR, sans reviewer parallèle ni duplication. Le reviewer envoie son verdict ou son diagnostic à l'exécuteur ; celui-ci transmet immédiatement le bilan au surveillant via `send_message_to_thread`. L'exécuteur doit annoncer explicitement l'`Entrée en revue` en précisant issue, PR, head, CI, preuve et périmètre contrôlé, puis traiter chaque retour actionnable et relancer les barrières touchées. Quand la CI GitHub est verte, que le reviewer Terra est favorable sur le head exact, que la preuve et les assets correspondent et que la PR est fusionnable, l'exécuteur envoie un `Go final` explicite au surveillant avec ces éléments. Pour une PR déjà en revue lors de l'entrée en vigueur de cette règle, l'exécuteur reprend la tâche Terra existante au lieu d'en créer une seconde.
