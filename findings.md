...
- La PR #28 de l’issue #9 a désormais la CI `33399387771` en échec uniquement sur `e2e/reglages.spec.ts:133` après trois tentatives ; neuf scénarios passent et lint, typage, Vitest et build restent verts. Le diagnostic montre que la liaison persistée `KeyZ` est correcte, mais que le bouton `Reprendre` conserve le focus après Annuler : `GestionnaireEntrees` ignore alors les touches ciblant un contrôle DOM. Le correctif minimal est le flou du bouton au début de `reprendreJeuActif`; l’exécuteur conserve la régression E2E, relance les barrières autorisées et pousse un nouveau head avant toute revue ou mise à jour de preuve.
...
| PR #26 acteur pirate | Ouverte — head resynchronisé, revue/CI en cours | Head `720ce89`, après fusion de `origin/main` dans #8 ; preuve structurelle en cours de régénération/revalidation et reviewer Terra existant doit recontrôler ce head. Le go `869f316` précédent est invalidé par l’évolution de `main`. |
...
| PR #28 réglages | Ouverte — correctif E2E en cours | Head `a1745e9`, P2 cookie corrigé et barrières locales vertes ; CI `33399387771` échoue uniquement sur `e2e/reglages.spec.ts:133` après trois tentatives. #9 diagnostique le mouvement après rechargement ; release et PR restent inchangées, sans reviewer ni go final. |
| PR #29 salle Colyseus | Ouverte — revue directe en cours | Head `b3a191d`, CI `33399155131` SUCCESS, PR `MERGEABLE/CLEAN`, diff ciblé et release/asset PNG alignés sur le head. #2 a annoncé l’Entrée en revue et lance le reviewer direct `gpt-5.6-terra` medium ; aucun go final ni merge avant verdict favorable. |
...
Dernier état connu : #22 est fusionnée sous `ae2a0f5`, #23 est fusionnée, #24 est fusionnée sous `d096f50`, et #27 vient d’être fusionnée sous `cfa0d654` avec #6 fermée. #26 attend le go formel de #8 ; #28 est bloquée par son E2E de rechargement ; #29 est en revue directe Terra après CI et preuve conformes ; #30 est bloquée par sa CI rouge. Les anciennes tentatives et le worktree `2acb` restent conservés pour ne pas écraser de travail utilisateur.
