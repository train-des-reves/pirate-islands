...
| PR #28 réglages | Ouverte — CI/release puis revue directe requises | Head `a1745e9`, P2 cookie corrigé et barrières locales vertes ; CI `33399387771` encore à vérifier sur ce head et la release reste à réaligner. La revue Carver `github-copilot/*` est invalide ; #9 doit lancer un reviewer direct `gpt-5.6-terra`, puis obtenir CI verte et go final. |
| PR #29 salle Colyseus | Ouverte — CI puis revue directe requises | Head `b3a191d`, diff ciblé après resynchronisation et release/description REST publiées depuis ce head ; CI `33399155131` à vérifier. La revue Huygens `github-copilot/*` est invalide ; #2 doit lancer un reviewer direct `gpt-5.6-terra` et obtenir le go final. |
