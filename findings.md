...
- Solution en cours : l’exécuteur #3 doit d’abord fusionner `origin/main` dans sa branche, puis téléverser les PNG issus de cette branche dans une pré-version GitHub dédiée (`gh release create`/`gh release upload`, ou l’interface Releases), vérifier `browser_download_url`/MIME et remplacer les images PR par des URLs absolues. La tâche planifiée surveille ces deux états avant toute revue ou fusion.
...
| PR #24 documentation | Ouverte — preuves à corriger | Branche `codex/protocole-assets`, issue #25 ; CI initiale verte mais revue Terra demande asset dédié à la branche, commande/scénario déterministe et `Closes #25`. |

Dernier état connu : la PR #23 est fusionnée et l’issue #4 fermée ; la PR #22 a une CI #10 verte mais attend les corrections P2, la résolution de conflit avec `main`, la preuve asset affichable puis une nouvelle revue Terra `medium`. La PR #24 documente le protocole et attend son asset dédié, son lien `Closes #25` et une nouvelle revue Terra. `main` a été mis à jour sur `cd739ef`. Les anciennes tentatives d’exécuteurs et le worktree `2acb` restent conservés pour ne pas écraser de travail utilisateur.
