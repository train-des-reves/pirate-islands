...
- La PR #28 de l’issue #9 a eu la CI `33399387771` en échec uniquement sur `e2e/reglages.spec.ts:133` après trois tentatives ; neuf scénarios passaient et lint, typage, Vitest et build restaient verts. Le diagnostic a confirmé que la liaison persistée `KeyZ` était correcte, mais que le bouton `Reprendre` conservait le focus après Annuler : `GestionnaireEntrees` ignorait alors les touches ciblant un contrôle DOM. Le correctif minimal `boutonReprendreJeu.blur()` est validé localement avec les barrières runtime, sans changement de liaison, cookie ou assertion ; le commit et le nouveau head restent à publier avant toute reprise de la preuve ou revue.
...
| PR #28 réglages | Ouverte — commit correctif à publier | Head distant connu `a1745e9`, P2 cookie corrigé ; le correctif ciblé `blur()` et les barrières locales sont verts, mais le commit/push et la CI du nouveau head restent requis. Release/PR inchangées, sans reviewer ni go final. |
| PR #29 salle Colyseus | Ouverte — contrôle final avant Go | Head `b3a191d`, CI `33399155131` SUCCESS, PR `MERGEABLE/CLEAN`, diff ciblé et release/asset PNG alignés sur le head. Reviewer direct `gpt-5.6-terra` medium favorable, sans finding ; #2 effectue le contrôle final et doit transmettre le Go final explicite avant toute fusion. |
...
Dernier état connu : #22 est fusionnée sous `ae2a0f5`, #23 est fusionnée, #24 est fusionnée sous `d096f50`, et #27 vient d’être fusionnée sous `cfa0d654` avec #6 fermée. #26 attend le go formel de #8 ; #28 a un correctif E2E local validé mais doit encore publier un nouveau head et repasser la CI ; #29 a reçu un verdict Terra favorable et effectue son contrôle final avant Go ; #30 est bloquée par sa CI rouge. Les anciennes tentatives et le worktree `2acb` restent conservés pour ne pas écraser de travail utilisateur.
