...
- Le blocage de la PR #49 sur la destruction du sloop pirate provenait de deux causes : d'une part, l'opérateur spread sur une instance de schéma Colyseus ne copiait pas les getters de coordonnées (x, z), générant des directions NaN rejetées par le serveur (4003: La direction de l'intention de tir est invalide). D'autre part, le ciblage E2E sélectionnait le premier pirate vivant disponible au lieu de l'équipage du sloop. Solution : extraction explicite des propriétés x, y, z et ciblage strict de equipageId. Conflit avec main (PR #43 pilotage) résolu en conservant les modes de jeu distincts. Commit de merge 8ef76df, CI GitHub Qualité et navigateur 100% SUCCESS, PR MERGEABLE et CLEAN.
...
| PR #28 réglages | Fusionnée — suivi terminé | Head `12fb3b8`, CI `33402672618` SUCCESS, revue `deepseek/deepseek-v4-pro` max favorable, release `mvp-2a-pr9-preuve-12fb3b8` alignée sur le head. Go final reçu ; fusion GitHub sous `c775a42`, issue #9 fermée, exécuteur archivé. |
...
| PR #49 pirates maritimes | Prête pour revue | Head `8ef76df`, CI `33733213079` SUCCESS, PR `MERGEABLE/CLEAN`, preuve alignée dans release `mvp-3e-issue13-reprise2-preuve` avec séquence GIF 4 étapes (patrouille, poursuite, attaque, destruction). |

Dernier état connu : #22 est fusionnée sous `ae2a0f5`, #23 est fusionnée, #24 est fusionnée sous `d096f50`, #26 est fusionnée sous `778dbd1`, #27 est fusionnée sous `cfa0d654`, #28 est fusionnée sous `c775a42` et #29 sous `15281a2d`, avec les issues #8, #6, #9 et #2 fermées. Après la fusion de #28, `main=c775a42` a évolué et la PR #30 est repassée en état UNKNOWN : le lead #5 doit resynchroniser sa branche avant la suite de sa revue. Les exécutants #7, #10, #11, #12 et #32 sont lancés. Les anciennes tentatives et le worktree `2acb` restent conservés pour ne pas écraser de travail utilisateur.
