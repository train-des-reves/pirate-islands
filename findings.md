...
- La PR #30 de l’issue #5 a publié six preuves bateau mais sa CI `33396133967` échoue sur la comparaison historique `e2e/monde.spec.ts` (14 533 pixels différents, ratio 0,02) alors que les trois scénarios bateau passent. Le head `dd5ae864` conserve encore une capture observationnelle/capsule à la place de `toHaveScreenshot` et n’est donc pas recevable : l’issue exige le bateau au quai, ce qui justifie une baseline monde déterministe mise à jour, sans affaiblir la barrière. Le Playwright local reste bloqué par `spawn EPERM` et ne vaut pas succès CI.
- La PR #28 de l’issue #9 a eu la CI `33399387771` en échec uniquement sur `e2e/reglages.spec.ts:133` après trois tentatives ; neuf scénarios passaient et lint, typage, Vitest et build restaient verts. Le premier correctif `blur()` n’a pas suffi : le second corrige le verrouillage du pointeur uniquement en mode E2E, sans modifier le chemin navigateur normal, et ses barrières locales sont vertes. Le nouveau commit reste à publier avant une nouvelle CI ; release, revue et Go restent interdits jusque-là.
- Après la fusion de la PR #29, la PR #26 de l’issue #8 est redevenue `CONFLICTING/DIRTY` contre `main` malgré la CI `33399797676` verte et la preuve alignée sur son head `480f14c`. L’exécuteur doit intégrer la référence `main` actuelle, résoudre au fichier près, relancer les barrières touchées, régénérer la preuve et reprendre le même reviewer Terra direct ; aucun merge n’est autorisé dans cet état.
...
| PR #26 acteur pirate | Ouverte — conflit de fusion à traiter | Head `480f14c`, CI `33399797676` SUCCESS et preuve dédiée cohérente, mais PR `CONFLICTING/DIRTY` après l’évolution de `main` par #29. #8 reprend la resynchronisation ; le reviewer Terra direct existant sera réutilisé après le nouveau head. Go précédent invalidé. |
...
| PR #28 réglages | Ouverte — CI du second correctif en attente | Le second correctif E2E est poussé après `41ecafc` ; le SHA exact reste à confirmer. Les releases visibles visent d’anciens heads ; aucune preuve cohérente, revue directe ou go final avant CI verte puis réalignement. |
...
| PR #30 bateau | Ouverte — correction de baseline requise | Head `dd5ae864`, CI `33396133967` rouge sur la comparaison monde historique ; les trois tests bateau passent, mais le head remplace encore `toHaveScreenshot` par une capture observationnelle et n’a pas de baseline mise à jour. #5 doit restaurer la comparaison et produire une baseline déterministe cohérente avec le bateau au quai, puis obtenir CI verte avant revue ou Go final. |

Dernier état connu : #22 est fusionnée sous `ae2a0f5`, #23 est fusionnée, #24 est fusionnée sous `d096f50`, #27 est fusionnée sous `cfa0d654` et #29 sous `15281a2d`, avec les issues #6 et #2 fermées. #26 est en resynchronisation après conflit ; #28 prépare la publication d’un second correctif E2E avant CI et preuve ; #30 doit restaurer sa comparaison historique et sa baseline avant CI/revue ; les anciennes tentatives et le worktree `2acb` restent conservés pour ne pas écraser de travail utilisateur.
